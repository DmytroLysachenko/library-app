...
const BorrowRequestsSection = async ({
  borrowRequestsPromise,
}: {
  borrowRequestsPromise: Promise<BorrowRecord[]>;
}) => {
  return (
    <section>
      ...
      <Suspense fallback={<EntryLoadingSkeleton />}>
        <BorrowRequestsEntries borrowRequestsPromise={borrowRequestsPromise} />
      </Suspense>
    </section>
  );
};

const BorrowRequestsEntries = async ({
  borrowRequestsPromise,
}: {
  borrowRequestsPromise: Promise<BorrowRecord[]>;
}) => {
  ...
  const requests = await borrowRequestsPromise;
